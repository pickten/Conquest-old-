...
data PT  = PT {ptID::Int, ptMain::PT'}
data Config = Cfg (Map.Map Int (Int, Int, Char, [Int])) [PT]

-- convenience methods
maxMoves = maxMoves' . ptMain
maxCarry = maxCarry' . ptMain
actions = actions' . ptMain
name = name' . ptMain
...
configParser = do
  ...
  where
    ...
    nodeStep = do
      ...
      (num,bl) <- getState
      putState (num + 1, bl)
      return (Cfg (Map.singleton num (own, piece, nodet, v)) [])
...
serializeConfig c@(Cfg mv ts) =
  ";; File autogenerated with the Conquest Config tool. \n;;See <https://github.com/pickten/Conquest/Conquest-Config-gen> for details."
  `T.append`
...
serializeMap :: Config -> Map Int (Int, Int, Char, [Int]) -> Text
serializeMap c m = foldr (\i a -> a `T.append` "\n" `T.append` serializeNode i m c) "" $ Map.keys m

serializeNode :: Int -> Map Int (Int, Int, Char, [Int]) -> Config  -> Text
serializeNode i m (Cfg _ c)= let
  (x, y, r, is) = m ! i
  in "(\n" `T.append` T.pack (show x) `T.append` "\n"
  `T.append` "; owner: \n" `T.append` name (c !! x) `T.append` "\n"
  `T.append` "; piece type: \n" `T.append` name (c !! y) `T.append` "\n"
